...
graph TD
    ...
    subgraph ComputeSpace [Solver Subsystem (Phase 4) & Research Subsystems (Phase 5 Implemented & Accepted)]
        ...
        BenchmarkWorker[Browser Benchmark Worker Adapter<br/>apps/web/src/workers/benchmark.worker.ts — Phase 5D Implemented & Accepted]
        BenchmarkEngine[Pure Benchmark Engine<br/>packages/benchmark — Phase 5 Implemented & Accepted]
...
- **Responsibilities (Implemented & Accepted Features):**
  ...
  - Hosts accepted M6 behavior: presentation-only `OrientationLegend` and disabled Drei `GizmoViewport` axis aid, semantic face-local move guidance, and a separate `useChallengeGenerator` lifecycle that creates solved-rooted candidates, certifies only `SolveSuccess.depth` with dedicated `IDA_STAR` Workers, and passes no solver move sequence to the accepted Play-baseline contract.
  - Hosts accepted M6.1 behavior: the existing Solver Worker and Play panel expose the four-algorithm portfolio, including optimal A* with H2 telemetry; M6 Challenge certification remains a separate `IDA_STAR`-only lifecycle.
...
#### M6 Play Orientation & Certified Challenge lifecycle (Accepted)
...
- **Responsibilities:**
  - Hosts the accepted Phase 4 classical graph-search baselines (BFS, Bidirectional BFS, and IDA* with the H2 two-slice PDB admissible heuristic) plus accepted M6.1 optimal graph-search A* with H2.
...
#### M6.1 A* solver extension (Accepted)
...
### 3.6. Research & Benchmark Harness (`packages/benchmark` & Browser Research Mode — Phase 5 Accepted Baseline; M6.1 Accepted Extension)
...
- **Core Responsibilities:**
  ...
  - **M6.1 benchmark integration:** The same version-1 runner, JSON/CSV schema, and browser Research Worker accept `A_STAR` alongside the three Phase 5 baseline algorithms; no historical Phase 5C report or raw artifact is rewritten.
...
- **Execution & Import Boundaries:**
  ...
  - **Node CLI Adapter (`packages/benchmark/src/cli.ts` — Phase 5B):** Isolated Node-only terminal runner with filesystem output (`node:fs`); unreachable from the browser-safe root engine entry.
  - **Browser Research Mode (`apps/web` — Phase 5D):** Dedicated background Web Worker (`apps/web/src/workers/benchmark.worker.ts`) importing the browser-safe engine and preventing heavy benchmark workloads from blocking the UI thread.
  - **Solver Worker vs. Benchmark Worker Protocol Distinctions:**
    - *Solver Worker (`solver.worker.ts`):* Reports incremental search telemetry (nodes expanded, depth bounds) for long-running single-case searches.
    - *Benchmark Worker (`benchmark.worker.ts`):* Executes atomic multi-case benchmark suites off-main-thread; emits no fabricated progress telemetry; performs Worker-side JSON/CSV serialization; lifecycle managed via monotonic `requestId` and host-side `worker.terminate()`.
...
For the M6 lifecycle, the same existing `START_SEARCH` / `SEARCH_COMPLETE` protocol is used by a separate hook-owned one-shot Worker. The challenge host reads only `result.depth`, discards the returned solution sequence, and requires generation/request identity to match before installing the certified challenge.
