...
- **Invariants Tested & Gates:**
  - Test fixtures generated by Phase 4A test-only Core-only exact-distance oracle covering exact distances $d \in [1 \dots 8]$.
  - BFS returns strictly shortest-path solutions of exact length $d$.
  - Bidirectional BFS returns optimal solutions matching BFS length $d$ under the provable lower-bound stopping rule.
  - IDA* returns solutions identical in length $d$ when Phase 4C admissible heuristic preflight is independently accepted.
