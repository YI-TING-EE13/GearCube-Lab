 ...
 * Throws a BenchmarkConfigError if the configuration is invalid.
 */
export function validateBenchmarkSuiteConfig(raw: unknown): BenchmarkSuiteConfig {
  if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) {
    throw new BenchmarkConfigError('BenchmarkSuiteConfig must be a non-null object');
  }
  ...
  for (const key of Object.keys(obj)) {
    if (!ALLOWED_CONFIG_KEYS.has(key)) {
      throw new BenchmarkConfigError(`Unknown configuration property: "${key}"`);
    }
  }

  // 1. schemaVersion
  if (obj['schemaVersion'] !== '1') {
    throw new BenchmarkConfigError(`Invalid schemaVersion: expected "1", got ${JSON.stringify(obj['schemaVersion'])}`);
  }

  // 2. suiteId
  if (typeof obj['suiteId'] !== 'string' || obj['suiteId'].length === 0) {
    throw new BenchmarkConfigError('suiteId must be a non-empty string');
  }

  // 3. seed
  if (typeof obj['seed'] !== 'string') {
    throw new BenchmarkConfigError('seed must be a string');
  }

  // 4. exactDepths
  const exactDepths = obj['exactDepths'];
  if (!Array.isArray(exactDepths) || exactDepths.length === 0) {
    throw new BenchmarkConfigError('exactDepths must be a non-empty array of numbers');
  }
  for (let i = 0; i < exactDepths.length; i++) {
    const d = exactDepths[i];
    if (typeof d !== 'number' || !Number.isInteger(d) || d < 1 || d > 8) {
      throw new BenchmarkConfigError(`exactDepths contains invalid depth ${JSON.stringify(d)}; must be an integer between 1 and 8`);
    }
    if (i > 0) {
      const prev = exactDepths[i - 1] as number;
      if (d <= prev) {
        throw new BenchmarkConfigError('exactDepths must be strictly increasing and unique with no duplicates');
      }
    }
  }
  ...
  if (typeof casesPerDepth !== 'number' || !Number.isInteger(casesPerDepth) || casesPerDepth < 1) {
    throw new BenchmarkConfigError('casesPerDepth must be an integer >= 1');
  }

  // 6. algorithms
  const algorithms = obj['algorithms'];
  if (!Array.isArray(algorithms) || algorithms.length === 0) {
    throw new BenchmarkConfigError('algorithms must be a non-empty array');
  }
  ...
  for (const alg of algorithms) {
    if (typeof alg !== 'string' || !ALLOWED_ALGORITHMS.has(alg as SolverAlgorithm)) {
      throw new BenchmarkConfigError(`Unsupported solver algorithm: ${JSON.stringify(alg)}`);
    }
    if (seenAlgorithms.has(alg)) {
      throw new BenchmarkConfigError(`Duplicate solver algorithm: "${alg}"`);
    }
  ...
  if (typeof warmupRuns !== 'number' || !Number.isInteger(warmupRuns) || warmupRuns < 0) {
    throw new BenchmarkConfigError('warmupRuns must be an integer >= 0');
  }

  // 8. measuredRuns
  const measuredRuns = obj['measuredRuns'];
  if (typeof measuredRuns !== 'number' || !Number.isInteger(measuredRuns) || measuredRuns < 1) {
    throw new BenchmarkConfigError('measuredRuns must be an integer >= 1');
  }
  ...
  if ('limits' in obj && obj['limits'] !== undefined) {
    ...
    if (typeof limits !== 'object' || limits === null || Array.isArray(limits)) {
      throw new BenchmarkConfigError('limits must be a non-null object');
    }
    const limitsObj = limits as Record<string, unknown>;

    for (const key of Object.keys(limitsObj)) {
      if (!ALLOWED_LIMITS_KEYS.has(key)) {
        throw new BenchmarkConfigError(`Unknown limits property: "${key}"`);
      }
    }
    ...
    if ('maxNodes' in limitsObj && limitsObj['maxNodes'] !== undefined) {
      ...
      if (typeof maxNodes !== 'number' || !Number.isInteger(maxNodes) || maxNodes < 1) {
        throw new BenchmarkConfigError('limits.maxNodes must be an integer >= 1');
      }
    }
    ...
    if ('maxDepth' in limitsObj && limitsObj['maxDepth'] !== undefined) {
      ...
      if (typeof maxDepth !== 'number' || !Number.isInteger(maxDepth) || maxDepth < 0) {
        throw new BenchmarkConfigError('limits.maxDepth must be an integer >= 0');
      }
    }
